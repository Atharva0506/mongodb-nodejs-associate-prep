# MongoDB Node.js Associate Prep

Short, exam-focused notes and practice material for MongoDB Associate Developer (Node.js).

## Purpose
- This repository is for educational and self-study use only.
- It summarizes concepts seen in official MongoDB practice questions.

## What is inside
- `mongodb-notes/`: Topic-wise notes (CRUD, aggregation, indexes, schema design, etc.)
- `offical-question.md`: Practice questions collected from the official MongoDB learning source.

# Credits

## Primary Sources
- MongoDB University (official learning platform):
  - https://learn.mongodb.com/
- MongoDB official documentation:
  - https://www.mongodb.com/docs/

## Notes
- Practice-question source used in this repo:
  - https://learn.mongodb.com/learn/course/associate-developer-node-practice-questions/prep-questions/practice-questions?client=customer
- This repository is maintained for educational and exam-preparation purposes.
- All product names and trademarks belong to their respective owners.



## License
- This project is licensed under the MIT License for original notes/content in this repository.
- Third-party trademarks and source materials remain the property of their respective owners.
- See `LICENSE`.
